...
export default {
    ...
    async execute(client: CustomClient, interaction: ChatInputCommandInteraction) {
        ...
        const failureResponse = (() => {
            switch (true) {
                ...
                case !!target && executor.roles.highest.comparePositionTo(target.roles.highest) <= 0 &&
                    ...
                    return {
                        content: "Tu ne peux pas bannir un membre ayant un rôle supérieur ou égal au tien.",
                        ephemeral: true,
                    }
                case !me?.permissions.has(PermissionFlagsBits.BanMembers):
                    return {
                ...
                case !!target && me && target.roles.highest.comparePositionTo(me.roles.highest) >= 0:
                    return {
                        content: "Je ne peux pas bannir ce membre car son rôle est trop élevé.",
                        ephemeral: true,
                    }
                default:
                    return null
        ...
        const memberToBan = target as GuildMember
        const reason = interaction.options.getString("raison") ?? "Aucune raison fournie."

        try {
            await memberToBan.ban({ reason })
            await interaction.reply({
                content: `${memberToBan.user.tag} a été banni. Raison : ${reason}`,
            })
        } catch (error) {
            await interaction.reply({
                content: "Une erreur est survenue lors du bannissement de ce membre.",
                ephemeral: true,
            })
        }
    },
